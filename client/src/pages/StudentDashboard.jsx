...
export default function StudentDashboard() {
  ...
  return (
    <div className="flex flex-col gap-10 animate-fade-in pb-20">
      ...
      {message && (
        <div className="bg-red-50 text-red-500 p-4 rounded-xl border border-red-100 flex items-center gap-3 shadow-sm">
        ...
        <>
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {orders.map((order) => (
                <article className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group" key={order.id || order._id}>
                  {/* Decorative corner accent based on status */}
                  <div className={`absolute top-0 right-0 w-16 h-16 transform translate-x-8 -translate-y-8 rotate-45 
                    ${order.status === 'delivered' ? 'bg-green-500' : order.status === 'cancelled' ? 'bg-red-500' : 'bg-primary-500'} opacity-20 group-hover:opacity-100 transition-opacity`}></div>
                  ...
                  {order.status === "delivered" && reviewOrderId !== (order.id || order._id) && (
                    <div className="mt-4 pt-3 border-t border-slate-100 flex justify-end">
                      <button 
                         ...
                         onClick={() => { setReviewOrderId(order.id || order._id); setFeedbackForm({ rating: 5, comment: "" }); }}
                      >
                         Leave Feedback
                      </button>
                    </div>
                  ...
                  {reviewOrderId === (order.id || order._id) && (
